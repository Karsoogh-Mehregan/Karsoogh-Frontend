...
export default function SelectedSubmissionId({ submissionId, onBack }: SubmissionFileProps) {
  ...
  useEffect(() => {
    const fetchDetailedSubmission = async () => {
      ...
      setError(null);
      try {
        const data = await apiClient.get<Submission>(`/exams/submissions/${submissionId}/`);
        setSubmission(data);
        if (data.grade !== null) {
  ...
  const handleSaveScore = async () => {
    ...
    setIsSaving(true);
    try {
      const updated = await apiClient.post<Submission>(`/exams/submissions/${submissionId}/`, {
        grade: parsedScore,
      });
  ...
  return (
    <div dir="rtl" className="flex flex-col h-screen max-h-[calc(100vh-4rem)] p-4 gap-4">
      <div className="flex items-center justify-between border-b border-white/10 pb-4 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-black text-white">بررسی پاسخ شناسه {submission?.id}</h3>
